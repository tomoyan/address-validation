...
export const BusinessDirectory: React.FC<BusinessDirectoryProps> = ({ businesses, address }) => {
  ...
  return (
    <div id="business-information-directory-section" className="bg-white rounded-[32px] border border-slate-200 shadow-sm p-6 sm:p-8 space-y-6">
      {/* Section Title Bar */}
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-100 pb-5">
          ...
          <div>
            <div className="flex items-center gap-2">
              <h3 className="text-xl font-extrabold text-slate-900 tracking-tight">
                Business & Tenant Breakdown
              </h3>
              <span className="px-2.5 py-0.5 rounded-full text-xs font-bold bg-blue-50 text-blue-700 border border-blue-200/70">
                {businesses.length} {businesses.length === 1 ? 'Entity' : 'Tenants'}
              </span>
            </div>
            <p className="text-xs text-slate-500 font-medium">
              Registered companies, headquarters, suites and commercial occupants
            </p>
          </div>
        </div>
        ...
        <div className="relative w-full sm:w-72">
          ...
          <input
            ...
            type="text"
            placeholder="Filter businesses or suites..."
            value={searchTerm}
            onChange={(e) => setSearchTerm(e.target.value)}
            className="w-full pl-10 pr-3.5 py-2.5 text-xs sm:text-sm bg-slate-50 border border-slate-200 rounded-xl focus:bg-white focus:outline-none focus:ring-4 focus:ring-blue-100 focus:border-blue-500 transition-all placeholder:text-slate-400 font-medium text-slate-700"
          />
        </div>
      </div>

      {/* Filter Category Chips */}
      {industryGroups.length > 2 && (
      ...
      {filteredBusinesses.length === 0 ? (
        <div className="rounded-2xl border border-slate-200 p-8 text-center space-y-2 bg-slate-50/60">
          <p className="text-sm font-bold text-slate-700">No businesses match your filter</p>
            ...
            return (
              ...
              >
                ...
                <div className="space-y-2.5">
                    ...
                    <div className="space-y-1 min-w-0">
                      <div className="flex items-center gap-1.5 flex-wrap">
                        ...
                        {biz.status === 'headquarters' && (
                          <span className="px-2.5 py-0.5 rounded-lg text-[11px] font-bold bg-purple-50 text-purple-700 border border-purple-200/70">
                            Global / Regional HQ
                          </span>
                        )}
                      </div>
                  ...
                  <div className="flex items-center gap-2 text-xs font-semibold text-slate-600 bg-slate-50 px-3 py-1 rounded-xl w-fit border border-slate-100">
                ...
                <div className="space-y-3 pt-3 border-t border-slate-100 text-xs">
                  ...
                  <div className="flex items-center justify-between gap-2 pt-1">
                    ...
                    <a
                      ...
                      target="_blank"
                      rel="noopener noreferrer"
                      className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-xs font-bold text-slate-700 bg-slate-50 hover:bg-white hover:shadow-sm border border-slate-200 transition-all ml-auto group/btn"
